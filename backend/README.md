# SplitMate Backend (Kotlin + Spring Boot)

유학생을 위한 **실시간 환율 & 더치페이 계산 도우미 – SplitMate** 백엔드입니다.

SOLO(1/N) / MENU BASED / GROUP(REST+WebSocket) 모드를 지원합니다.
...
- CAD → KRW **실시간 환율 변환**
- 여러 명이 함께 입력/확인하는 **GROUP 모드(방/실시간 브로드캐스트)**
...
키가 없으면 자동 환율 대신 **수동 입력 fallback**으로 동작합니다.
...
- `EXCHANGE_API_KEY`가 비어 있으면 자동 환율 선택 시 수동 입력 단계로 안내됩니다.
...
> Layered Architecture + Hexagonal Mindset
>
...
com.splitmate
 ...
 │   ├─ conversation
 │   │    ├─ flow        # 대화 흐름(유스케이스)
 │   │    ├─ model       # ConversationContext/Output/Step 등 모델
 │   │    └─ payload     # MenuPayload 파서/공용 입력 묶음
 │   └─ group            # Room/Member 상태 및 서비스
 │
 ...
 ├─ adapter
 │   ├─ console          # CLI 입출력
 │   ├─ fx               # 외부 환율 API Provider
 │   ├─ http             # REST Controller/DTO
 │   │    └─dto
 │   └─ websocket        # STOMP Controller
 │
 └─ config               # DI 조립 / Spring Config

```

- **domain**: 순수 비즈니스 규칙 (I/O 없음)
- **application**: 흐름/유스케이스 조립, 도메인 호출
- **adapter**: 콘솔/HTTP/WebSocket/외부 API 등 입출력
- **config**: 의존성 조립
...
- [x]  앱 시작 시 환영 메시지와 간단한 사용 설명을 출력한다.
- [x]  첫 질문은 항상 “총 결제 금액을 입력해주세요”로 시작한다.

**테스트**

- [x]  `ConversationFlow.start()` 호출 시, 첫 출력이 환영/안내 + “총 결제 금액” 질문인지 검증한다.
...
- [x]  팁 %, 인원 수, 환율 등 숫자 입력이 필요한 모든 단계에 대해 위 패턴이 동작하는지 각각 테스트한다.
...
- [x]  `% 모드` 선택 후 `15` 입력 → `tipMode = PERCENT`, `tipValue = 15` 저장.
- [x]  `$ 모드` 선택 후 `10` 입력 → `tipMode = ABSOLUTE`, `tipAmount = 10` 저장.
...
- [x]  “1) N분의 1 2) 메뉴별 계산” 중 하나를 고르게 한다.
...
- [x]  총액 27.40, 세금 2.60, 팁 10%(총액+세금 기준) / 인원 2명 → 기대 CAD 1인당 금액이 맞는지 계산식 검증.
- [x]  팁이 0인 경우, 세금만 포함한 금액이 정상적으로 나뉘는지.
- [x]  세금/팁 모두 0일 때, 단순히 `총액 / N`이 되는지.
- [x]  소수점이 긴 결과에서 반올림이 의도한 대로 되는지.
...
- [x]  provider가 예외를 던지는 상황(실패 케이스) 테스트
...
- [x]  소수점이 많은 CAD 값 × 환율에서 KRW 반올림이 제대로 되는지.
...
- [x]  `MenuItem` 생성 시 음수/0 가격을 허용하지 않는지 검증한다.
...
- [x]  `participants`가 1명일 때 → 전액이 그 사람 subtotal에 더해지는지 테스트.
- [x]  `participants`가 N명일 때 → 메뉴 가격이 `N`으로 나뉘어 각 사람 subtotal에 더해지는지 테스트.
- [x]  `participants`가 빈 리스트일 때 예외가 나는지 테스트.
...
- [x]  `SplitResult.total`가 사람별 total 합과 일치하는지 테스트.
...
- [x]  환율 없이 CAD만 보고 싶을 때는 KRW 계산을 생략한다.
...
- [x]  방 상태를 표현하는 `RoomStatus` enum 정의 (`OPEN`, `CLOSED` 등)
...
- [x]  멤버 미가입, 없는 방, context 타입 오류 등 에러 케이스 테스트 보완
