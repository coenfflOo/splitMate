...
class TipTest {
    ...
    fun `퍼센트 팁은 기준 금액의 퍼센트를 계산한다`() {
        ...
        val tip = Tip(TipMode.PERCENT, BigDecimal("15")) // 15%

        val tipAmount = tip.calculate(base)
    ...
    fun `고정 금액 팁은 그대로 사용한다`() {
        val base = Money.of("100.00", Currency.CAD)
        val tip = Tip(TipMode.ABSOLUTE, BigDecimal("5.25"))
    ...
    fun `퍼센트 팁은 0 이상 100 이하만 허용한다`() {
        assertFailsWith<IllegalArgumentException> {
            Tip(TipMode.PERCENT, BigDecimal("-1"))
        }
        assertFailsWith<IllegalArgumentException> {
            Tip(TipMode.PERCENT, BigDecimal("101"))
        }
    }
}
